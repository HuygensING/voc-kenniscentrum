...
sub returnresults {	# creates HTML page from template file
	...
	if ($query->param('help') == 1) {
		...
		$h{header} = 'S.V.P. hieronder zoektermen intypen';
		$h{help} = $SEARCH_TITLE if $SEARCH_TITLE;
	} elsif (@none) {
		$h{header} = 'Er zijn geen pagina\'s gevonden die '.$casesearch.' <b>'.$bare_query.'</b> bevatten';
		$h{subheader} = '<br>Er zijn geen pagina\'s gevonden die <B>'.(join " ", @none).' bevatten</B>'.$commonterms.$search_sources.$weight_tip;
		$h{help} = $SEARCH_TITLE if $SEARCH_TITLE;
	} elsif (!$results) {
		$h{header} = 'Er zijn geen pagina\'s gevonden die '.$casesearch.' <b>'.$bare_query.'</b> bevatten';
		...
		my $documents;
		if ($first == $last) {
			$documents = 'Treffer <b>'.($first+1).'</b>';
		} else {
			$documents = 'Treffers <b>'.($first+1).'-'.($last+1).'</b>';
		}
		$h{help} = $RESULTS_TITLE if $RESULTS_TITLE;
		my $s = ($totalmatches == 1 ? "" : "s");

		$h{header} = $documents.' van de <b>'.$results.'</b>,<br> waarbij gezocht is op <b>'.$bare_query.'</b>';

		# $h{header} = $documents.' van de <b>'.$results.'</b>, waarbij gezocht is op <b>'.$bare_query.'</b> met in totaal <b>'.$totalmatches.'</b>'.$casesearch.' treffer'.$s.$subsearch_info;
		...
		$h{subsearch} = '<INPUT type="checkbox" name="pq" value="'.$previous_queries.$previous_query_str.'"'.$checked.'>Zoek binnen de resultaten' if $SEARCH_RESULTS;
		$h{form_link} = $FORM_LINK;
	}
	...
	foreach ((@sortedanswers)[$first..$last]) {
		...
		if ($SORT_BY eq "Matches") {
			$scoreA = '<b>Treffers:</b> '.$matches{$_};
			$scoreB = "<b>$score</b> ".$finalscores{$_};
			$scoreC = '<b>Laatst gewijzigd:</b> '.get_date($f_date_db{$_});
			$scoreD = '<b>Bestandsgrootte:</b> '.$f_size_db{$_}.'KB';
		} elsif ($SORT_BY eq "Scores") {
			$scoreA = "<b>$score</b> ".$finalscores{$_};
			$scoreB = '<b>Treffers:</b> '.$matches{$_};
			$scoreC = '<b>Laatst gewijzigd:</b> '.get_date($f_date_db{$_});
			$scoreD = '<b>Bestandsgrootte:</b> '.$f_size_db{$_}.'KB';
		} elsif ($SORT_BY eq "Dates")  {
			$scoreA = '<b>Laatst gewijzigd:</b> '.get_date($f_date_db{$_});
			$scoreB = '<b>Treffers:</b> '.$matches{$_};
			$scoreC = "<b>$score</b> ".$finalscores{$_};
			$scoreD = '<b>Bestandsgrootte:</b> '.$f_size_db{$_}.'KB';
		} else {
			$scoreA = '<b>Treffers:</b> '.$matches{$_};
			$scoreB = '<b>Bestandsgrootte:</b> '.$f_size_db{$_}.'KB';
			$scoreC = "<b>$score</b> ".$finalscores{$_};
			$scoreD = '<b>Laatst gewijzigd:</b> '.get_date($f_date_db{$_});
		}
	...
	if ($currentpage == 1) {
		...
		if ($lastpage > $currentpage) {
			$h{next} = " &nbsp; <A href=\"$SEARCH_URL?".$FORM_INPUT_NAME."=".$query_str."&display=".$RESULTS_PER_PAGE."&p=2".$subsearch_string.$options.$sortby."\" title=\"Naar pagina 2\" alt=\"To page 2\"> $NEXT</A>\n";
		} else {
			$h{next} = "";
		}
	} elsif ($currentpage == $lastpage) {
		$h{previous} = "<A href=\"$SEARCH_URL?".$FORM_INPUT_NAME."=".$query_str."&display=".$RESULTS_PER_PAGE."&p=".($lastpage-1).$subsearch_string.$options.$sortby."\" title=\"Naar pagina ".($lastpage-1)."\" alt=\"Naar pagina ".($lastpage-1)."\">$PREVIOUS </A> &nbsp;\n";
		$h{next} = "";
	} else {
		$h{previous} = "<A href=\"$SEARCH_URL?".$FORM_INPUT_NAME."=".$query_str."&display=".$RESULTS_PER_PAGE."&p=".($currentpage-1).$subsearch_string.$options.$sortby."\" title=\"Naar pagina ".($currentpage-1)."\" alt=\"Naar pagina ".($currentpage-1)."\">$PREVIOUS </A> &nbsp; \n";
		$h{next} = " &nbsp; <A href=\"$SEARCH_URL?".$FORM_INPUT_NAME."=".$query_str."&display=".$RESULTS_PER_PAGE."&p=".($currentpage+1).$subsearch_string.$options.$sortby."\" title=\"Naar pagina ".($currentpage+1)."\" alt=\"Naar pagina ".($currentpage+1)."\"> $NEXT</A>\n";
	}
	if ($lastpage == 1) {
		$h{navbar} = "";
	} else {
		for (1..$lastpage) {
			...
			if ($_ != $currentpage) {
				$h{navbar} .= " &nbsp;<A href=\"$SEARCH_URL?".$FORM_INPUT_NAME."=".$query_str."&display=".$RESULTS_PER_PAGE."&p=".$_.$subsearch_string.$options.$sortby."\" title=\"Pagina ".$_."\" alt=\"Pagina ".$_."\">$resultspage</A>&nbsp; \n";
			} else {
...
#	if (timestr(timediff(new Benchmark, $t0)) =~ /^(.+)$/i && !@none && $results) { # wallclock, usr, and sys times
		$h{searchtime} = '<br>Benodigde tijd: <b>'.($1 == 1 ? "$1 seconde" : "$1 secondes")."</b>";
		if ($1 >= $SPEED_TIP_TIME) {$h{searchtime} .= '<br>NB: Gebruik minder algemene termen om sneller te zoeken.'; }
	} else {
		$h{searchtime} = "";
	}
